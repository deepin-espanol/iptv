var ErrorPlugin = Clappr.ContainerPlugin.extend({
  ...
  show: function() {
    var $ = Clappr.$
    this.hide();
    var txt = (this.options.errorPlugin && this.options.errorPlugin.text) ? this.options.errorPlugin.text : 'Error de canal';
    this._err = $('<div>')
      .css({
        'position': 'absolute',
        'z-index': '999',
      ...
      .append($('<p>').html('Vuelva a intentar este canal en unos minutos o informe a su proveedor de IPTV.')
        .css({
          'font-size': '120%',
          'margin': '15px',
        }));
